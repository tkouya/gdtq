...
/*
 * CUDA 13 notes:
 *   - Defined __NV_NO_VECTOR_DEPRECATION_DIAG before any include, to
 *     silence the double4 / long4 / ... deprecation warnings that
 *     would otherwise fire through the gqd_real typedef.
 *   - Replaced the deprecated cudaThreadSynchronize() with the
 *     recommended cudaDeviceSynchronize() throughout.
 *   - startTimer / endTimer / getLastCudaError (used below via
 *     cutilCheckMsg) are now supplied by cuda_header.cu using CUDA
 *     events, because helper_timer.h / helper_cuda.h from the old
 *     CUDA Samples are no longer available with CUDA 13.
 */
...
float device_basic(gtd_real* h_in1, gtd_real* h_in2, gtd_real* h_out, const unsigned int numElement,
                        OPERATOR op = ADD,
                        const unsigned int numBlock = 128,
                        const unsigned int numThread = 128) {
	return device_basic_template(h_in1, h_in2, h_out, numElement, op, numBlock, numThread);
...
/* ---- float-based overloads (gds/gts/gqs) ---- */
float device_basic(gds_real* h_in1, gds_real* h_in2, gds_real* h_out, const unsigned int numElement,
                        OPERATOR op = ADD,
                        const unsigned int numBlock = 128,
                        const unsigned int numThread = 128) {
        return device_basic_template(h_in1, h_in2, h_out, numElement, op, numBlock, numThread);
...
float device_basic(gts_real* h_in1, gts_real* h_in2, gts_real* h_out, const unsigned int numElement,
                        OPERATOR op = ADD,
                        const unsigned int numBlock = 128,
                        const unsigned int numThread = 128) {
        return device_basic_template(h_in1, h_in2, h_out, numElement, op, numBlock, numThread);
...
float device_basic(gqs_real* h_in1, gqs_real* h_in2, gqs_real* h_out, const unsigned int numElement,
                        OPERATOR op = ADD,
                        const unsigned int numBlock = 128,
                        const unsigned int numThread = 128) {
        return device_basic_template(h_in1, h_in2, h_out, numElement, op, numBlock, numThread);
