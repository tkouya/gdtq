...
#ifndef __GQS_BASIC_CU__
#define __GQS_BASIC_CU__
...
gqs_real sloppy_add(const gqs_real &a, const gqs_real &b)
{
	...
	renorm(s0, s1, s2, s3, t0);
        ...
        return make_qs(s0, s1, s2, s3);
}


// 2025-12-25(Wed) T.Kouya
// Branch free algorithm
// void Add4(const float x[4], const float y[4], float z[4]) {
//static inline void c_qd_add_bf(const float *a, const float *b, float *c)
__host__ __device__ 
gqs_real bf_add(const gqs_real &a, const gqs_real &b)
{
	...
	//c[2] = c12;
	//c[3] = d12;
	return make_qs(a10, b11, c12, d12);
}
...
{
        return make_qs( -a.x, -a.y, -a.z, -a.w );
}
...
gqs_real mul_pwr2(const gqs_real &a, float b) {
        return make_qs(a.x * b, a.y * b, a.z * b, a.w * b);
}
...
gqs_real sloppy_mul(const gqs_real &a, const gqs_real &b)
{
	...
	renorm(p0, p1, s0, s1, s2);
        ...
        return make_qs(p0, p1, s0, s1);
	
}
// 2025-12-24(Wed) T.Kouya
// Branch free algorithm
// void Mul4(const float x[4], const float y[4], float z[4]) {
//static inline void c_qd_mul_bf(const float *a, const float *b, float *c)
__device__
gqs_real bf_mul(const gqs_real &a, const gqs_real &b)
{
	float a0, b0, c0, d0, e0, f0, g0, h0, i0, j0, k0, l0, m0, n0, o0, p0;
	float         c1, d1, e1, f1, g1,     i1, j1,         m1, n1;
	float     b2, c2,     e2, f2,     h2, i2,             m2;
	float a3, b3, c3, d3, e3, f3, g3, h3;
	float         c4, d4, e4, f4;
	float                 d5;
	float         c6,     d6;
	float     b7, c7, d7;
...
{
	return make_qs(ldexp(a.x, n), ldexp(a.y, n), 
